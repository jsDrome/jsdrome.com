...
class SidebarContent extends Component {
  ...
  render() {
    ...
    return <div className={classes.root}>
      ...
      {list.slice(1).map((item, i) => item.links.length ? <NestedList key={i} title={`${item.title} ${process.env.NODE_ENV === 'development' ? `(${item.links.length})` : '' }`} className={classes.nestedList} open={item.title === selectedList}>
